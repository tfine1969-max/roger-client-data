...
function AttachmentRow({ type, label, attachment, onAttachmentUpload }) {
  return (
    <div className="flex items-center justify-between border border-border rounded-sm px-2 h-8 bg-background">
      ...
      {attachment ? (
        <a href={attachment.file_url} target="_blank" rel="noopener noreferrer"
          className="text-[10px] text-ocean hover:underline font-medium">View</a>
      ) : (
        <label className="text-[10px] text-ocean hover:underline font-medium cursor-pointer">
          Upload
  ...
  return (
    <div className="grid grid-cols-2 gap-6">
      ...
      <div className="bg-card border border-border rounded-lg p-4 flex flex-col">
        <h2 className="text-[10px] font-bold text-navy uppercase tracking-wider mb-3">Attachments</h2>
        <div className="space-y-3 flex-1">
          {investments.map((inv) => {
            ...
            return (
              <div key={inv.id}>
                <p className="text-[9px] font-bold text-ocean uppercase tracking-wider border-b border-border pb-1 mb-1.5">{label}</p>
          ...
          {riskProducts.map((rp) => {
            ...
            return (
              <div key={rp.id}>
                <p className="text-[9px] font-bold text-teal uppercase tracking-wider border-b border-border pb-1 mb-1.5">{label}</p>
                <div className="space-y-1.5">
                  <AttachmentRow type={quoteKey} label="Quote PDF" attachment={attachments.find(a => a.attachment_type === quoteKey)} onAttachmentUpload={onAttachmentUpload} />
                  <AttachmentRow type={formKey} label="Application Form" attachment={attachments.find(a => a.attachment_type === formKey)} onAttachmentUpload={onAttachmentUpload} />
                </div>
      ...
      <div className="bg-card border border-border rounded-lg p-4 flex flex-col">
        <h2 className="text-[10px] font-bold text-navy uppercase tracking-wider mb-3">PDF Document</h2>
        <div className="flex-1">
          {data.proposal_pdf_url ? (
            <div className="flex items-center justify-between bg-green-50 border border-green-200 rounded-sm px-3 py-2.5 mb-3">
              <div className="flex items-center gap-2">
                <FileText className="w-4 h-4 text-green-700" />
                <div>
                  <p className="text-xs font-semibold text-green-900">PDF Ready</p>
              ...
              <a href={data.proposal_pdf_url} target="_blank" rel="noopener noreferrer"
                className="flex items-center gap-1 px-2.5 py-1.5 bg-green-700 text-white rounded-sm text-xs font-medium">
                <Download className="w-3 h-3" /> Download
              </a>
            </div>
          ) : (
            <div className="bg-blue-50 border border-blue-200 rounded-sm px-3 py-2.5 mb-3">
              <p className="text-xs text-blue-900">No PDF generated yet.</p>
            </div>
          )}
        </div>
        <button onClick={onGeneratePdf}
          className="w-full bg-navy hover:bg-ocean text-white py-2 text-xs font-medium uppercase tracking-wide flex items-center justify-center gap-2 rounded-sm mt-auto">
      ...
      <div className="bg-card border border-border rounded-lg p-4 flex flex-col">
        <h2 className="text-[10px] font-bold text-navy uppercase tracking-wider mb-3">Distribution</h2>
        <div className="space-y-2 mb-4 flex-1">
          {[
            ...
            <div key={label} className="flex items-center justify-between px-3 py-2 bg-muted/50 rounded-sm">
              <div className="flex items-center gap-2">
                {signed
        ...
        <button
          ...
          className={`w-full py-2 text-xs font-medium uppercase tracking-wide flex items-center justify-center gap-2 rounded-sm transition-colors mb-2 ${
        ...
        {!hasSig && (
          <p className="text-[10px] text-muted-foreground text-center mb-2">Sign above to proceed</p>
        )}

        {signingLinkLocal && (
          <div className="p-2.5 bg-green-50 border border-green-200 rounded-sm">
            <p className="text-[9px] font-semibold text-green-800 uppercase tracking-wide mb-1.5">Client Signing Link</p>
            <div className="flex items-center gap-2">
              <input
                readOnly
                value={signingLinkLocal}
                className="flex-1 text-[10px] text-navy bg-white border border-border px-2 py-1.5 rounded-sm outline-none truncate"
              />
              <button
                onClick={() => navigator.clipboard.writeText(signingLinkLocal)}
                className="text-[10px] px-3 py-1.5 bg-navy text-white rounded-sm whitespace-nowrap"
              >
                Copy
              </button>
            </div>
