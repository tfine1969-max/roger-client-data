...
export default function GryphonUpload({ onImported }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await base44.functions.invoke('importGryphonFile', {
        file_url,
      ...
      setStatus('success');
      setMessage(`Imported ${res.data.rows_imported} holdings for ${uploadMonth}.`);
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-muted-foreground">
        Upload a Gryphon Unit Holder Report (.xlsx) to import monthly holdings. All values are in ZAR.
      </p>
      <form onSubmit={handleSubmit} className="bg-white border rounded-lg p-6 space-y-5">
        ...
        <label className="flex items-center gap-2 text-sm cursor-pointer select-none">
          ...
          />
          <span>Replace existing Gryphon data for this month</span>
        </label>
        <Button
          type="submit"
      ...
      <DeleteMonthData provider="gryphon" />
    </div>
  );
}